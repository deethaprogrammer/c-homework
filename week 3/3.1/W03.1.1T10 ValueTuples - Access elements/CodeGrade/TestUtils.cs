...
static class TestUtils
{
    ...
    public static void PrintIsOverriddenMethod(Type baseType, Type derivedType, Type[] parameterTypes, string methodName)
    {
        ...
        if (baseMethod == null)
        {
            PrintMethodNotFoundInClass(baseType, methodName);
            return;
        }
        if (derivedMethod == null)
        {
            PrintMethodNotFoundInClass(derivedType, methodName);
            return;
        }
    ...
    {
        Console.WriteLine($"{interfaceType.Name} is NOT implemented by {clsType}: "
            + !interfaceType.IsAssignableFrom(clsType));
    }
    ...
    public static void PrintAllEnumValues<T>() where T : Enum
    {
        ...
        Console.WriteLine($"Printing all values of enum {typeof(T).Name}:");
    ...
    public static void PrintTupleParameterNames(Type clsType, string methodName)
    {
        var methods = clsType.GetMethods(
            BindingFlags.Public | BindingFlags.NonPublic |
            BindingFlags.Instance | BindingFlags.Static)
        ...
        foreach (var method in methods)
        {
            Console.Write($"Checking method with signature: ");
            PrintMethodSignature(method);
            ...
            foreach (var param in method.GetParameters())
            {
                var tupleAttr = param
                    ...
                    .FirstOrDefault(attr => attr.AttributeType == typeof(System.Runtime.CompilerServices.TupleElementNamesAttribute));

                if (tupleAttr != null)
                {
                    var namesArg = tupleAttr.ConstructorArguments.FirstOrDefault();
                    var names = namesArg.Value as IReadOnlyList<CustomAttributeTypedArgument>;

                    Console.WriteLine($"Tuple parameter '{param.Name}' has named elements:");
                ...
                {
                    Console.WriteLine($"Parameter '{param.Name}' is not a named tuple.");
                }
            }
