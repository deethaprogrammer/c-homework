static class Program
{
    static void Main()
    {
        ...
        do 
        {
            Console.WriteLine("What direction do you want to walk to? (N/E/S/W)");
            string moveing = Console.ReadLine().ToUpper();
            if (player.TryMoveTo(player.CurrentLocation.GetLocationAt(moveing)))
            {
                Console.WriteLine(player.CurrentLocation.Compass());
            }
        } while (player.CurrentLocation != World.Locations[^1]);
